  ...
  function FancyTable($header, $consulta)
{
    ...
    foreach($consulta as $row)
    {
        ...
        $this->Cell($w[3],12,$row['citPaciente'],'LR',0,'C',$fill);
        $this->Cell($w[4],12,$row['citMedico'],'LR',0,'C',$fill);
        $this->Ln();
        $fill = !$fill;
    }
    // Línea de cierre
...
SELECT * FROM citas WHERE citfecha BETWEEN '$fecha1' AND '$fecha2' ORDER BY citfecha");
$consulta ->execute();  
...
$pdf->Cell(0,8,"Informe de citas",0,1,'L',true);
$pdf->PrintChapter($fecha1, $fecha2);
$header = array('Fecha', 'Hora Inicio', 'Hora Final', 'Paciente', utf8_decode('Médico'));
