...
$consulta = $conexion -> prepare("
	SELECT * FROM localidad ORDER BY $order $sort"); 

$consulta ->execute();
$consulta = $consulta ->fetchAll();
if(!$consulta){
	$mensaje .= 'NO HAY LOCALIDADES PARA MOSTRAR';
}
$select = 5;
...
$consulta = $conexion -> prepare("
	SELECT * FROM localidad ORDER BY $order $sort LIMIT $RegistroInicial, $CantidadRegistrosPorPagina "); 
...
if (!empty($_POST['btnBuscar1'])) {
    ...
    if (!empty($_POST['PatronNombre'])) {
        $consulta=BuscarLocalidad($_POST['PatronNombre']);
        if (!empty($consulta)) {
		...
		<div class="wrapp">
			<?php include 'plantillas/nav.php'; ?>
				<article>
					...
					<a class="agregar" href="agregarLocalidad.php">Agregar Localidad</a>
					</table>
					<table class="tabla">
						  <tr>
                            <?php echo "<th><a href='localidades.php?order=idLocalidad&sort=".$sort."'>Nº".$arrow."</a></th>";?>
							<th>Nombres</th>
							<th colspan="2">Opciones</th>
						  </tr>
						<?php foreach ($consulta as $Sql): ?>
						<tr>
                            <?php echo "<td>". $Sql['idLocalidad']. "</td>"; ?>
							<?php echo "<td>". utf8_encode($Sql['nombre']). "</td>"; ?>
                            <?php echo "<td class='centrar'>"."<a href='actualizarLocalidad.php?idLocalidad=".$Sql['idLocalidad']."' class='editar'>Editar</a>". "</td>"; ?>
                            <?php echo "<td class='centrar'>"."<a href='eliminar_localidad.php?idLocalidad=".$Sql['idLocalidad']."' class='eliminar'>Eliminar</a>". "</td>"; ?>
						</tr>
						<?php endforeach; ?>
					</table>
            ...
            if (!empty($PaginaAnterior)) {
                ?> 
                <a href="localidades.php<?php echo $PaginaAnterior; ?>" >Anterior </a>
            <?php } else { ?>
                Anterior
            <?php } ?>
            ...
            <?php for ($j = 1; $j <= $PaginasAMostrar; $j++) { ?>
                * <a href="localidades.php?page=<?php echo $j; ?>"> <?php echo $j; ?> </a>
            <?php } ?>
            ...
            if (!empty($PaginaSiguiente)) {
                ?> 
                * <a href="localidades.php<?php echo $PaginaSiguiente; ?>" >Siguiente </a>
            <?php } else { ?>
                Siguiente
            <?php } ?>		
				</article>
