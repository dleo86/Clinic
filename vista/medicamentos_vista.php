...
$consulta = $conexion -> prepare("
	SELECT * FROM medicamento ORDER BY $order $sort"); 
...
$consulta = $conexion -> prepare("
	SELECT * FROM medicamento ORDER BY $order $sort LIMIT $RegistroInicial, $CantidadRegistrosPorPagina "); 
...
if (!empty($_POST['btnBuscar1'])) {
    ...
    if (!empty($_POST['PatronNombre'])) {
        $consulta=BuscarMedicamento($_POST['PatronNombre']);
        if (!empty($consulta)) {
		...
		<div class="wrapp">
			<?php include 'plantillas/nav.php'; ?>
				<article>
					...
					<a class="agregar" href="agregarMedicamento.php">Agregar Medicamento</a>
					</table>
					<table class="tabla">
						  <tr>
                            <?php echo "<th><a href='medicamentos.php?order=idmedicamento&sort=".$sort."'>Nº".$arrow."</a></th>";?>
							<th>Nombres</th>
							<th colspan="2">Opciones</th>
						  </tr>
						<?php foreach ($consulta as $Sql): ?>
						<tr>
                            <?php echo "<td>". $Sql['idmedicamento']. "</td>"; ?>
							<?php echo "<td>". utf8_encode($Sql['mednombre']). "</td>"; ?>
                            <?php echo "<td class='centrar'>"."<a href='actualizarMedicamento.php?idmedicamento=".$Sql['idmedicamento']."' class='editar'>Editar</a>". "</td>"; ?>
                            <?php echo "<td class='centrar'>"."<a href='eliminar_medicamento.php?idmedicamento=".$Sql['idmedicamento']."' class='eliminar'>Eliminar</a>". "</td>"; ?>
						</tr>
						<?php endforeach; ?>
					</table>
            ...
            if (!empty($PaginaAnterior)) {
                ?> 
                <a href="medicamentos.php<?php echo $PaginaAnterior; ?>" >Anterior </a>
            <?php } else { ?>
                Anterior
            <?php } ?>
            ...
            <?php for ($j = 1; $j <= $PaginasAMostrar; $j++) { ?>
                * <a href="medicamentos.php?page=<?php echo $j; ?>"> <?php echo $j; ?> </a>
            <?php } ?>
            ...
            if (!empty($PaginaSiguiente)) {
                ?> 
                * <a href="medicamentos.php<?php echo $PaginaSiguiente; ?>" >Siguiente </a>
            <?php } else { ?>
                Siguiente
            <?php } ?>		
				</article>
