...
$consulta = $conexion -> prepare("
	SELECT * FROM especialidades ORDER BY idespecialidad");
...
$consulta = $conexion -> prepare("
	SELECT * FROM especialidades ORDER BY $order $sort limit $RegistroInicial, $CantidadRegistrosPorPagina");
...
if (!empty($_POST['btnBuscar1'])) {
    ...
    if (!empty($_POST['PatronNombre'])) {
        $consulta=BuscarEspecialidad($_POST['PatronNombre']);
        if (!empty($consulta)) {
	...
	<section class="main">
		...
		<div class="wrapp">
				...
				<article>
					...
					<table>
					 <form method="post">
						...
						<a class="agregar" href="agregarEspecialidades.php">Agregar Especialidad</a>
					</table>	
						<table class="tabla">
						  <tr>
                            <?php echo "<th><a href='especialidades.php?order=idespecialidad&sort=".$sort."'>Nº".$arrow."</a></th>";?>
                            <?php echo "<th><a href='especialidades.php?order=espNombre&sort=".$sort."'>Nombre".$arrow."</a></th>";?>
            ...
            if (!empty($PaginaAnterior)) {
                ?> 
                <a href="especialidades.php<?php echo $PaginaAnterior; ?>" >Anterior </a>
            <?php } else { ?>
                Anterior
            <?php } ?>
            ...
            <?php for ($j = 1; $j <= $PaginasAMostrar; $j++) { ?>
                * <a href="especialidades.php?page=<?php echo $j; ?>"> <?php echo $j; ?> </a>
            <?php } ?>
            ...
            if (!empty($PaginaSiguiente)) {
                ?> 
                * <a href="especialidades.php<?php echo $PaginaSiguiente; ?>" >Siguiente </a>
            <?php } else { ?>
                Siguiente
            <?php } ?>		
				</article>
